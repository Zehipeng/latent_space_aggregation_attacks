...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the task-specific 2-key smoke gate and 50-key forgery P0"
    )
    ...
    if args.preflight_only:
        print(json.dumps({"stage": "forgery_budget_selection_pilot", "status": checked["status"]}, indent=2))
        return
    run_id = args.run_id or datetime.now(timezone.utc).strftime("p0_forgery_%Y%m%dT%H%M%SZ")
    result = run_p0(
        config=checked["config"], assets_lock=checked["assets"], output_root=args.output_root,
        run_id=run_id, smoke_only=args.smoke_only,
        project_root=Path(__file__).resolve().parents[2], expected_task="forgery",
    )
