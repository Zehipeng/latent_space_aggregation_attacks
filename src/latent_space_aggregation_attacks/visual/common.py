...
EXPECTED = {
    "experiment_version": "visual_ablation_v1", "method_label": "FR-LA",
    "watermark": "ringid", "model_setting": MODEL,
    "key_ids": ["key_000", "key_001"], "N_values": [1, 5, 25],
    ...
    "difference": "semantic_forgery_rgb_absolute_unamplified",
}
...
def load_settings(path: str | Path) -> dict[str, Any]:
    ...
    if settings != EXPECTED:
        raise ValueError("Visual settings differ from the user-approved visual_ablation_v1 contract")
    return settings
    ...
    for task in ("forgery", "removal"):
        for factor in (("lambda", "N") if task == "forgery" else ("lambda", "N", "beta")):
            for value in settings[f"{factor}_values"]:
                ...
                cid = f"{task}_N{n}_lambda{int(lam)}" + (f"_beta{beta:g}" if task == "removal" else "")
