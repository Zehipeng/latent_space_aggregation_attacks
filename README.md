...
由独立进程在攻击完成后执行。历史P0允许在线检测并早停，但v1.22不再执行P0。

## 目录

- `configs/current/`：唯一当前配置；含v1.22正式与2-key smoke。
...
- `src/.../archive/`：历史P0运行时，只读且不进入v1.22正式路径。
...
- `scripts/validate_tree_ring.py`：正式GPU回归门禁。`validate_batching.py`仅用于追溯v1.17失败门禁，不属于v1.22启动链。
...
| `validate_batching.py` | 追溯v1.17标量与批量失败门禁 | 归档v1.17 config、资产锁 | 历史诊断JSON，不进入v1.22门禁 |
...
- v1.22沿用v1.21的标量执行：攻击优化、DDIM反演和参考VAE编码的batch size均固定为1。v1.17的FP16真批量设置未通过GPU等价性门禁，不得用于v1.22。
- v1.22不持久保存任何参考、检查点或最终输出PNG。选中的参考PNG在全部主攻击输出完成并逐项校验后生成清理清单并删除；最终输出在离线评价及哈希校验后由既有spool清理删除。数值结果、清单、哈希、表格和统计图永久保留。
...
每个run严格使用协议第11节目录。选中参考PNG和`evaluation_spool/`分别只在协议规定的
完整性门禁通过后自动清理，且都会先写入精确清单；其他目录不得随之删除。
...
| 协议允许自动清理 | 通过消费门禁的选中参考PNG；通过最终评价行数/hash门禁的全部`evaluation_spool/` PNG |
...
```bash
python -m pytest -q
python scripts/operations/inspect_run.py --config configs/current/formal_v1p22.yaml
python scripts/operations/inspect_run.py --config configs/current/smoke_v1p22_2key.yaml
```
...
python scripts/validate_tree_ring.py \
  ...
  --output /root/autodl-tmp/outputs/regression/tree_ring_v1p22.json \
  --offline
```
...
v1.22 GPU验收先运行Tree-Ring回归门禁，再进入正式2-key smoke。smoke使用
`configs/current/smoke_v1p22_2key.yaml`以逐张方式跑满150步；通过、审阅ETA并显式批准后，
