...
本项目是 `formal_protocol_v1.18` 的正式代码项目。权威协议增量快照位于
`docs/protocols/formal_protocol_v1.18.md`，其基线为v1.17；历史项目
...
由独立进程在攻击完成后执行。历史P0允许在线检测并早停，但v1.18不再执行P0。

## 目录

- `configs/current/`：唯一当前配置；含v1.18正式与2-key smoke。
...
- `src/.../archive/`：历史P0运行时，只读且不进入v1.18正式路径。
...
- `scripts/validate_tree_ring.py`：正式GPU回归门禁。`validate_batching.py`仅用于追溯v1.17失败门禁，不属于v1.18启动链。
...
| `validate_batching.py` | 追溯v1.17标量与批量失败门禁 | 归档v1.17 config、资产锁 | 历史诊断JSON，不进入v1.18门禁 |
...
- v1.18标量执行：攻击优化、DDIM反演和参考VAE编码的batch size均固定为1。v1.17的FP16真批量设置未通过GPU等价性门禁，不得用于v1.18。
...
python scripts/operations/inspect_run.py --config configs/current/smoke_v1p18_2key.yaml
```
...
python scripts/run_formal.py \
  --config configs/current/formal_v1p18.yaml \
  --smoke-config configs/current/smoke_v1p18_2key.yaml \
  --assets-lock local_assets/assets.lock.json \
  --tree-ring-regression-report /root/autodl-tmp/outputs/regression/tree_ring_v1p18.json \
  --task forgery \
  --run-id <run_id> \
  --offline \
  --smoke-only
...
v1.18 GPU验收先运行Tree-Ring回归门禁，再进入正式2-key smoke。smoke使用
`configs/current/smoke_v1p18_2key.yaml`以逐张方式跑满150步；通过、审阅ETA并显式批准后，
