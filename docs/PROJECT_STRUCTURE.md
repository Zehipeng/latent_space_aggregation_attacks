...
The repository separates the active v1.22 execution path from historical material.

```text
latent_space_aggregation_attacks/
├─ configs/
│  ├─ current/                 # only runnable v1.22 formal/smoke configs
│  ├─ assets/                  # AutoDL inventory template
│  └─ archive/                 # v1.15/v1.16, P0, diagnostics, templates
├─ scripts/
│  ├─ run_formal.py            # single formal CLI
│  ├─ validate_batching.py     # archived v1.17 batch diagnostic utility
...
└─ docs/
   ├─ protocols/               # current v1.22 snapshot
...
Anything under an `archive/` directory is excluded from the v1.22 formal path.
