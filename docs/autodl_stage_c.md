# AutoDL阶段C：离线资产锁定与历史v1.14 P0记录

> v1.15状态：用户已冻结伪造与移除正式预算均为1500步、移除beta网格为`[1.0,1.5,2.0]`，并取消后续P0与50-key固定预算确认。本页中的P0命令只用于追溯v1.14运行，不得在v1.15下重新执行或覆盖旧结果。新的正式AutoDL运行指令须在`run_formal_batch.py`完整实现并通过本地测试后另行交付。
