...
- Origin Mode: plan
- Protocol Version: `formal_protocol_v1.13`
...
- 正式实验固定运行至预算选择实验后冻结的 $T_{\mathrm{formal}}$，不得依据检测结果早停、调参或选择中间图像。当前 15000 仅为预算选择实验的候选上限，不是已经冻结的正式预算。
...
- 正式主预算：$T_{\mathrm{formal}}$ 次梯度更新，由第 6.1 节预算选择实验确定并在正式实验开始前通过新协议版本冻结；当前候选上限为 15000。
...
**目的**：观察本文方法在候选上限 15000 步内每 100 步的累计攻击成功率，用独立开发数据决定正式固定预算 $T_{\mathrm{formal}}$。P0 是允许检测器反馈的协议设计实验，不是正式黑盒攻击实验。

- 方法：只运行本文多参考潜空间聚合的伪造方法和移除方法；不在 P0 中比较 Jain 或 Can。
- 参数：$N=5$、$\lambda=10^4$、移除$\beta=1.0$、候选最大步数$T_{\max}=15000$；模型固定为正式实验的跨模型分支（SD2-base目标/SD1.4 VAE代理），其余生成、预处理和优化设置与正式主设置一致。
...
- 覆盖：三种水印、仅跨模型设置、伪造与移除两个任务。使用既有预注册序列的前50个逻辑pilot key，编号 `pilot_key_000` 至 `pilot_key_049`；每种水印由manifest映射为各自的真实密钥对象。在线早停阶段共计 $3\times2\times50=300$ 个攻击单元，不运行P0同模型版本。
...
- 在线检测：在 100, 200, …, 15000 步先把攻击tensor规范化为最终保存所用的512×512 RGB 8-bit图像，再调用目标检测器。样本第一次成功后记录 `first_success_step` 并立即早停；尚未成功的样本继续至下一检查点或 15000 步。
- 成功定义与正式实验一致；ASR 分母仍为攻击前满足资格条件的样本，必须同时报告 `eligible_n/50`。
- 第 $t$ 步累计 ASR 定义为：`first_success_step <= t` 的资格样本数除以资格样本总数。已经早停的成功样本在后续步数继续计为成功，因此可得到 100–15000 步的完整累计 ASR 曲线。
...
2. 若 15000 步累计 ASR 仍不理想，正式实验不得启动；必须重新检查移除目标、损失权重和方法有效性，不能直接把失败隐藏在正式实验中。
3. 若 15000 步前已经达到用户认可的效果，可提出更小的、100 的整数倍作为 $T_{\mathrm{candidate}}$。
4. 冻结前必须在相同50个pilot key、三种水印和跨模型设置上执行一次固定预算确认，共300个单元：从头运行至 $T_{\mathrm{candidate}}$，攻击过程中不调用检测器、不早停，只在结束后离线计算ASR，输出 `pilot_fixed_budget_confirmation.csv`。这一步用于检验在线累计ASR是否高估固定预算效果；不增加同模型确认。
...
7. 预算、选择依据、完整 P0 曲线和固定预算确认结果必须写入新的协议版本后才能开始正式 200-key 实验；P0 的 50-key 结果不得并入正式主表、显著性检验或正式 ASR。
...
P0 单独保存在 `实验结果/budget_selection_pilot/<run_id>/`，至少包含协议/配置快照、50-key pilot manifest、`reference_selection_control.csv`、`pilot_first_success.csv`、按任务/水印/模型设置分组的 `pilot_asr_by_step.csv`、全部300张攻击终点PNG、伪造/移除累计ASR曲线PNG、`pilot_fixed_budget_confirmation.csv`、时间记录、校验和及一份预算选择总结。P0不持久保存参考PNG，且不得复制进正式 run 或与正式 200-key 指标合并。
...
5. 先对 P0 执行同设置 2-key smoke；通过后由同一工作流直接启动 50-key P0，得到每 100 步累计 ASR。
6. 用户审阅完整 P0 曲线并提出 $T_{\mathrm{candidate}}$；完成同50-key、无早停的固定预算确认后，确认全局 $T_{\mathrm{formal}}$，随后升级协议版本、记录选择依据并冻结预算。该门槛完成前禁止启动任何正式 200-key 实验。
...
- P0 启动命令先自动生成仅含 `pilot_key_000`、`pilot_key_001` 的 smoke 子配置；通过后直接进入 50-key P0。P0 smoke 保持每 100 步检测和早停，以验证其在线曲线链路。
