...
- Origin Mode: plan
- Protocol Version: `formal_protocol_v1.12`
...
- 在线检测：在 100, 200, …, 1500 步先把攻击tensor规范化为最终保存所用的512×512 RGB 8-bit图像，再调用目标检测器。样本第一次成功后记录 `first_success_step` 并立即早停；尚未成功的样本继续至下一检查点或 1500 步。
...
P0 单独保存在 `实验结果/budget_selection_pilot/<run_id>/`，至少包含协议/配置快照、50-key pilot manifest、`reference_selection_control.csv`、`pilot_first_success.csv`、按任务/水印/模型设置分组的 `pilot_asr_by_step.csv`、`pilot_fixed_budget_confirmation.csv`、时间记录、校验和及一份预算选择总结。P0不持久保存参考图、攻击结果图或曲线图片，且不得复制进正式 run 或与正式 200-key 指标合并。
