...
- Origin Mode: plan
- Protocol Version: `formal_protocol_v1.9`
...
- 所有子种子通过 SHA-256 稳定派生：对 UTF-8 字符串 `formal_protocol_v1.9|205|<namespace>|<identifiers>` 计算 SHA-256，取前 8 字节按无符号大端整数解释，再对 $2^{63}-1$ 取模。禁止使用 Python `hash()`、当前时间、进程号或未记录的系统随机源。
- 命名空间固定为：
...
- 数据隔离：P0的100个pilot key、伪造攻击目标身份和水印参考清单不得与正式200-key测试集的对应角色重叠。作为公开无标签辅助输入的`val2017` clean-prior池允许按第4.1节规则跨任务、跨阶段复用，但不得把P0攻击输出、检测结果或结果驱动筛选带入正式实验。使用`master_seed=205`，子种子额外加入`budget_pilot`命名空间。
