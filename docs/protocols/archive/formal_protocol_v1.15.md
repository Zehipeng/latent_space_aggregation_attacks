...
- Origin Mode: plan
- Protocol Version: `formal_protocol_v1.15`
- Protocol Date: 2026-09-01
- Verification Status: `UNVERIFIED`（本文件是正式实验预注册协议，不是实验结果）
- Source Draft: `C:\Users\dell\Desktop\实验设置.docx`
- Research Code: `C:\Users\dell\Desktop\codex学习文档\latent_space_aggregation_attacks`

### v1.15 变更记录（2026-09-01）

- 用户在审阅已完成的伪造P0后，明确冻结正式伪造攻击与正式移除攻击的任务级固定预算均为1500步，即`T_forgery_formal=1500`、`T_removal_formal=1500`。同一任务内所有迭代方法、水印、模型设置和单因素条件共享对应任务预算；正式攻击禁止在线检测与早停。
- 用户明确取消后续P0与50-key固定预算确认，允许直接进入正式实验的批次级2-key smoke门禁。已有P0及10-key移除诊断继续只读保留，不并入正式统计；1500步移除预算不是由已完成的移除P0固定预算确认得出，属于用户批准的设计决定，须作为证据限制披露。
- 移除强度网格由`{0.5,1.0,2.0}`升级并冻结为`{1.0,1.5,2.0}`。正式主设置仍为`beta=1.0`；`beta=1.5`和`beta=2.0`用于E5单因素实验，其他移除实验除非研究beta，否则固定`beta=1.0`。
- 正式检测统计量轨迹统一保存步骤`0,100,200,...,1500`；1500步也是每150步可视化PNG规则的自然终点。协议版本、正式配置、smoke配置、条件注册表和测试必须同步更新后才可启动正式批次。

### v1.14 变更记录（2026-08-31）

- 根据已完成的2-key smoke，将伪造与移除P0拆成两个入口、两个resolved config、两次smoke和两个独立run；两类任务不得再写入同一`pilot_first_success.csv`、累计ASR表、曲线或总结。
- 伪造P0候选上限锁定为`T_forgery_max=3000`，每100步检测并在首次成功后早停；50-key伪造P0为150个攻击单元，2-key smoke为6个攻击单元。
- 移除P0继续使用独立的候选上限`T_removal_max=15000`，等待下述参数诊断完成后再决定是否升级其正式主参数；50-key移除P0同样为150个攻击单元，2-key smoke为6个攻击单元。
- 新增独立的移除参数诊断：预注册序列前10个pilot key、三个水印、仅SD2.0目标/SD1.4 VAE跨模型、Proposed、`N=5`、`lambda=1e4`、`beta=1.5`、固定3000步。该诊断不中途检测、不早停、不保存检查点PNG，只在结束后报告ASR、`l2`、`linf`、LPIPS、SSIM、PSNR和阈值归一化优化进度百分比。
- 优化进度百分比以攻击前为0%、刚好跨过正式拒绝阈值为100%；Tree-Ring/RingID使用`100*(p_final-p_initial)/(0.05-p_initial)`，Gaussian Shading使用`100*(a_initial-a_final)/(a_initial-a_threshold)`，不截断超过100%的结果。
- 该10-key结果只用于判断`beta=1.5`是否值得进入后续移除P0或协议升级，不并入P0预算曲线、正式ASR或论文主表。观察结果前，正式主设置仍为`beta=1.0`。
- 预算选择与冻结改为按任务分开：伪造和移除分别提出候选预算并完成各自50-key固定预算确认；每个任务内部所有迭代方法、水印和模型设置共享同一预算，不允许为同一任务中的方法单独挑步数。

### v1.13 变更记录（2026-08-31）

- 用户明确将50-key P0在线预算选择的候选上限从1500提高为`T_max=15000`，仍每100步以规范RGB 8-bit表示检测、首次成功后早停，并每50步覆盖保存resume状态。15000只是P0候选上限，不是已冻结的`T_formal`。
- P0继续禁止保存参考PNG：有效参考仅在内存中生成/检测/编码，控制manifest记录生成seed、接受分数和规范PNG SHA-256；续跑按seed重新生成并复核。
- 用户恢复P0攻击结果与ASR曲线图的持久留存。2-key smoke必须保存12张攻击终点PNG；50-key P0必须保存300张攻击终点PNG，并生成伪造与移除各1张累计ASR曲线PNG。所有路径与SHA-256进入CSV、报告及`checksums.sha256`。
- P0报告必须分别记录`persistent_reference_image_count=0`、`persistent_attack_image_count`、`persistent_asr_curve_image_count`及总`persistent_image_count`；缺失或哈希错误的攻击结果不能视为完整单元，原命令重跑时只重算对应单元。

### v1.12 变更记录（2026-08-31）

- 用户明确P0不需要保存任何参考图、攻击结果图或ASR曲线图片，只保留可计算预算所需的评估指标与实验数据。P0永久图像文件数必须为0；正式200-key实验的图像留存规则不变。
- 参考有效性选择仍在内存中使用规范化512×512 RGB 8-bit表示检测，并在`reference_selection_control.csv`记录候选顺序、检测分数、接受状态、生成seed和规范PNG的SHA-256，但不写出参考PNG。续跑需要参考时按相同prompt/seed重新生成，并同时复核接受状态与SHA-256。
- P0攻击终点仍转换为与原协议相同的RGB 8-bit表示完成在线检测并记录最终SHA-256，但不落盘保存PNG。累计ASR只保存`pilot_asr_by_step.csv`，不生成曲线PNG。
- 每50步覆盖式`resume_state`中的临时tensor继续保留以支持中断恢复；它不是永久评价图像，完整批次验收后可由清理清单处理。P0报告必须写明`persistent_image_count=0`。

### v1.11 变更记录（2026-08-31）

- 用户因P0全量等待时间过长，明确将预算选择与后续固定预算确认的pilot规模从100个key调整为50个key；使用既有预注册序列的`pilot_key_000`至`pilot_key_049`，不得依据已观察到的成功或失败结果换key、重排或重新抽样。
- P0在线早停阶段与固定预算确认阶段各由600个攻击单元调整为300个攻击单元（50 keys × 3水印 × 2任务）。2-key smoke门禁、跨模型设置、`N=5`、`lambda=10000`、`beta=1`、学习率、阈值、参考有效性规则和1500步初始候选上限均不变。
- `formal_protocol_v1.10`下已启动的100-key P0标记为不完整诊断批次并只读保留，不得与v1.11结果合并。v1.11复用v1.10已冻结的100-key资产manifest，但只消费其前50个预注册pilot key；继续保留`formal_protocol_v1.9|205|...`种子命名空间，避免查看结果后重采样。
- 缩减样本量降低预算曲线精度：每个任务/水印分组的分母最多为50，必须报告`eligible_n`与Wilson区间；P0仍仅用于选择迭代预算，不进入正式200-key主结果或显著性结论。

### v1.10 变更记录（2026-08-31）

- Tree-Ring生成与检测参数正式锁定为`channel=0`、`radius=16`、`p_value<=0.05`，与Jain复现代码及历史Tree-Ring实验保持一致；`radius=10`的未完成P0批次只保留为诊断记录，不得并入预算选择。
- 每个key和水印预注册64个有序参考候选。准备阶段按固定顺序检测并选取最先通过正式阈值的25张作为`R_25`，从而保证`R_1`、`R_5`和`R_25`中的每张参考图均为有效目标密钥水印图。候选顺序、全部已检测候选、接受/拒绝分数和最终选择必须写入控制manifest；不足25张时批次失败，禁止人工挑选或改变候选顺序。
- 上述参考有效性检测属于攻击前的数据准备与有效性控制，不属于攻击者查询。正式攻击进程仍不得加载或调用检测器，攻击仅消费已经冻结且通过完整性校验的参考库。
- P0每100步的检测和早停统一使用与最终文件完全相同的512×512 RGB 8-bit表示，禁止在浮点tensor上早停后再用PNG重新定义结果。
- PIL/COCO输入预处理锁定为与历史Jain复现一致的RGB转换、BICUBIC短边缩放至512、中心裁剪512×512及`[-1,1]`归一化，禁止直接把非方形图像拉伸为正方形。
- 本次升级发生在查看`formal_protocol_v1.9`未完成P0诊断结果之后。为禁止结果出现后重新抽取有利样本，v1.10继续使用`formal_protocol_v1.9|205|...`种子派生前缀并保持v1.9的COCO角色分配与前25个提示词顺序；每个key新增的候选索引25–63只用于完成预注册参考候选池。

### v1.9 变更记录（2026-08-29）

- 伪造目标与移除攻击的非配对干净先验统一使用MS-COCO 2017 `val2017`：200张预注册伪造目标来自该split，移除clean-prior池使用该split全部5000张图像。
- 明确允许两类不同攻击任务之间复用同一张公开干净图像。200张伪造目标因此是5000张移除clean-prior池的子集；该跨任务角色重叠不合并两类攻击的样本、ASR、质量指标或FID。
- 保留移除clean prior的关键边界：它不得是待移除水印目标的语义配对干净图，不得依据检测或攻击结果筛选；每个key的25张clean prior仍由预注册manifest确定并在200个key之间互不重叠。
...
- 正式实验按任务固定运行1500步，即$T_{\mathrm{forgery}}=T_{\mathrm{removal}}=1500$，不得依据检测结果早停、调参或选择中间图像。
- 检测和质量评价由独立离线进程在全部攻击完成后执行。
- 参考候选、提示词、种子和顺序必须在检测前固定。数据准备阶段只允许按第4.2节的统一规则选取固定顺序中最先通过正式阈值的候选，不得人工挑选、改变顺序或对同一候选重生成；候选原始检测率与全部拒绝记录必须作为实验控制报告。
- “首次达到成功的步数”只允许在对应任务P0中计算。正式实验的ASR、质量、成本和输出选择只使用该任务冻结预算的最终输出；第10.2节轨迹仅作离线诊断。
...
- 伪造目标：MS-COCO 2017 `val2017`中的200张互不重复图像，由预注册manifest固定。
- 非配对干净先验：使用MS-COCO 2017 `val2017`全部5000张图像，按稳定顺序切分为200个互不重叠的25张组并分配给200个key。
- 允许跨攻击任务复用数据：上述200张伪造目标是5000张clean-prior池的子集，同一张公开干净图像可以在伪造任务中作为攻击目标、在移除任务中作为clean prior。该重叠不改变两类任务的攻击输入、成功定义、质量参照和集合级评价均须分别计算。
- `data_role_overlap.csv`必须逐图记录COCO image ID、原始路径、是否为伪造目标、对应伪造key、是否为clean prior、对应clean-prior key与组内索引；正式运行前验证伪造目标数为200、clean-prior唯一图数为5000、两集合交集恰为200，并验证200个clean-prior组两两不相交且完整覆盖5000张图。
- 所有图像统一为 $512\times512$ RGB；预处理、颜色空间、插值和像素范围必须写入 resolved config。

### 4.2 密钥、参考图与嵌套抽样

- 密钥数：$K=200$，统一编号 `key_000` 至 `key_199`。
- 每个密钥和水印预注册64张有序候选并逐张生成、检测；按候选顺序选取最先通过正式阈值的25张作为有效参考库。若64张中不足25张有效，数据准备失败且不得启动攻击。
- Tree-Ring候选必须使用`channel=0`、`radius=16`；Tree-Ring/RingID候选以`p_value<=0.05`为有效，Gaussian Shading候选以锁定的正式bit-accuracy阈值为有效。
- `reference_selection_control.csv`必须记录候选索引、提示词哈希、生成seed、检测统计量、正式阈值、接受状态、选择状态、选择后参考索引和图像SHA-256；选定的25张图必须持久保存并由攻击入口复核哈希。
- 参考集合严格嵌套：
  \[
...
- 所有子种子通过 SHA-256 稳定派生。为保持查看诊断结果前已经预注册的样本不变，v1.10继续对 UTF-8 字符串 `formal_protocol_v1.9|205|<namespace>|<identifiers>` 计算 SHA-256，取前 8 字节按无符号大端整数解释，再对 $2^{63}-1$ 取模；该固定前缀记录为`seed_namespace_version=formal_protocol_v1.9`。禁止使用 Python `hash()`、当前时间、进程号或未记录的系统随机源。
- 命名空间固定为：
...
- 非配对干净先验与待移除水印目标、参考图无语义配对关系，不得针对目标人工选择或依据检测/攻击结果筛选。它可以与另一项伪造任务的COCO目标图像发生预注册的跨任务角色重叠；这种重叠不构成移除目标的配对clean信息。
...
- Tree-Ring正式参数：`channel=0`、`radius=16`、圆形mask、ring pattern、50步生成、50步反演、接受阈值`p_value<=0.05`。
...
- 正式主预算：$T_{\mathrm{forgery}}=1500$、$T_{\mathrm{removal}}=1500$，均由用户在本版本中明确冻结。
...
所有正式单因素实验均覆盖200个密钥、三个水印、同模型和跨模型两个设置。除被研究因素外，其余参数保持主设置。重复条件只运行一次并由统一结果索引复用。已有P0仅作为历史预算诊断保留，不再构成正式实验启动门槛。

### P0：已结束的任务分离迭代预算诊断

**目的**：分别观察伪造和移除方法每100步的累计攻击成功率，用独立开发数据决定任务级正式固定预算 $T_{\mathrm{forgery}}$ 与 $T_{\mathrm{removal}}$。P0 是允许检测器反馈的协议设计实验，不是正式黑盒攻击实验。

- 方法：伪造P0只运行本文伪造方法，移除P0只运行本文移除方法；两者使用不同脚本、配置、run_id与结果目录，不在 P0 中比较 Jain 或 Can。
- 参数：两类P0均固定$N=5$、$\lambda=10^4$；伪造候选上限为3000步；移除P0在10-key参数诊断完成及用户确认前保持$\beta=1.0$和15000步候选上限。模型均固定为跨模型分支（SD2-base目标/SD1.4 VAE代理）。
- P0对每个key和水印使用64个预注册候选中的最先5张有效参考；五张必须全部通过对应水印正式阈值，否则该批次失败。选择规则和全部已检测候选写入`reference_selection_control.csv`，选定图像的SHA-256进入数据完整性记录，但P0不持久保存图像文件。
- 覆盖：每次P0均覆盖三种水印、仅跨模型设置和前50个预注册pilot key。伪造P0与移除P0各150个攻击单元，分别运行，不运行P0同模型版本。
- 数据隔离：P0的50个pilot key、伪造攻击目标身份和参考候选清单不得与正式200-key测试集的对应角色重叠。作为公开无标签辅助输入的`val2017` clean-prior池允许按第4.1节规则跨任务、跨阶段复用；P0攻击输出和攻击成功结果不得带入正式实验。参考有效性选择只能使用统一的预注册候选顺序和正式阈值。使用`master_seed=205`，子种子额外加入`budget_pilot`命名空间。
- 在线检测：每100步先把攻击tensor规范化为最终保存所用的512×512 RGB 8-bit图像，再调用目标检测器。伪造最多运行至3000步，移除最多运行至其当前锁定候选上限；样本第一次成功后记录`first_success_step`并立即早停。
- 成功定义与正式实验一致；ASR 分母仍为攻击前满足资格条件的样本，必须同时报告 `eligible_n/50`。
- 第 $t$ 步累计 ASR 定义为：`first_success_step <= t` 的资格样本数除以资格样本总数。伪造与移除各自产生一张只含本任务的完整累计ASR曲线。
- 该累计曲线回答“在线检测并早停时，截至某步有多少样本曾成功”，不等于“所有样本持续优化到该步时的固定预算 ASR”；成功后继续优化可能重新跌出阈值，因此不能只凭累计曲线直接声明某个固定预算的ASR。
- 每个P0 run输出自己的`pilot_first_success.csv`、`pilot_asr_by_step.csv`和唯一一张`pilot_<task>_asr_curve.png`；禁止把两个任务合并到同一文件。
- P0 仅报告 ASR、资格样本数、首次成功步数和实际攻击时间；不承担正式图像质量、FID、错误密钥或方法优越性结论。由于存在在线检测和早停，其时间不得与正式固定预算成本直接比较。

**v1.15冻结决定**：

1. 用户明确冻结$T_{\mathrm{forgery}}=1500$与$T_{\mathrm{removal}}=1500$，并取消剩余P0及两个50-key固定预算确认。
2. 该决定允许在本协议、代码、配置、资产和正式manifest一致后直接进入每批次必需的2-key正式smoke；smoke通过后由同一编排流程进入200-key正式运行。
3. 已有P0只作为预算设计背景保存。其在线早停累计ASR不能解释为1500步固定预算ASR，也不能用于正式图像质量或方法优越性结论。
4. 1500步移除预算缺少50-key固定预算确认，因此正式结果分析必须披露该预注册限制；不得在观察正式结果后按水印、方法或模型追加步数。
...
- Jain 与本文使用 $\lambda=10^4$、共同的任务级$T_{\mathrm{forgery}}$；Can 使用 $\gamma=1$。
...
- 仅本文移除方法。
- $\beta\in\{1.0,1.5,2.0\}$，固定 $N=5$、$\lambda=10^4$。
...
- β网格新增：2400个唯一输出。
- 常规变换：6000个唯一输出。
- 总计：34800个唯一攻击/变换输出，其中21600个为固定任务级预算的迭代攻击。
- 不再为全部200 keys永久保存最终图或中间检查点图像。每个实验条件只永久保留三个固定可视化key；迭代方法每150步保存PNG，并额外确保保存对应任务最终预算PNG。
- 其余 197 个 key 的最终图只在独立评价完成前进入临时 `evaluation_spool/`；检测、质量和 FID 特征写入持久的最终指标文件并通过哈希/行数校验后，临时图按协议清除。
- 全部200 keys永久保存对应任务冻结预算的最终评价指标和最终输出哈希；不永久保存每50步数值日志。
...
正文成本主表报告`attack_compute_time`，复现材料同时提供总墙钟、I/O与离线评价时间。Can和常规变换按其真实一次性计算过程计时，不与任务级固定预算优化混为一谈。

## 8. 运行状态、可视化检查点与最终评价

- 步骤 0 只在 manifest 中记录输入、配置和哈希，不保存攻击指标或检测结果。
- 每 50 步原子覆盖一次临时 `resume_state`，仅用于中断续跑；它不是永久攻击日志，不进入论文分析，实验单元完成并校验后即可按契约清除。
- 仅为3个预注册可视化key永久保存最终图；迭代方法每150步保存无损PNG，并额外保存对应任务最终预算PNG。
- 每150步永久PNG只用于观察优化过程和制作定性图，不据此早停或选择输出。
- 为生成第10.2节规定的五组检测统计量曲线，仅对Proposed的E3–E5相关条件，将全部200 keys在步骤0、100、200、…、对应任务最终预算的图像临时写入`curve_checkpoint_spool/`；若最终预算不是100的整数倍，额外写入最终步。攻击进程不得加载或调用检测器。
...
- 其余197个key的任务级最终预算输出先原子写入`evaluation_spool/`。独立评价器对最终图计算目标/错误密钥检测与质量指标。
...
- 最终总结、统计检验和论文表图全部基于对应任务冻结预算输出；不得按key选择“最佳检查点”。
...
本节规定论文最终必须生成的表图。第10.1节表格、最终ASR、质量和成本结果均来自对应任务冻结预算输出；第10.2节另行规定五组离线检测统计量轨迹。
...
4. **不同 $\beta$ 实验表（E5，仅移除）**
   ...
   只报告Proposed移除方法，固定$N=5$和$\lambda=10^4$，$\beta$按1.0、1.5、2.0排列。
...
本节“两类主攻击”指Proposed伪造与Proposed移除。各图使用步骤0、100、200、…、对应任务冻结预算的离线检测结果；横轴为迭代步数。每张图采用2行 × 3列布局：行分别为`Model=SDv1.4`同模型和`Model=SDv2.0`跨模型，列分别为Tree-Ring、RingID、Gaussian Shading。每个面板恰好三根超参数曲线，不把模型或水印再编码成额外曲线。
...
5. **移除—不同$\beta$**：$\beta\in\{1.0,1.5,2.0\}$三根线，固定$N=5$、$\lambda=10^4$；文件名`removal_beta_detector_trajectory.png`。
...
- 已完成P0的每100步在线早停累计ASR曲线，以及实际存在的预算确认记录；不存在的固定预算确认不得补写或伪造；
...
伪造与移除P0分别保存在`实验结果/budget_selection_pilot/forgery/<run_id>/`和`.../removal/<run_id>/`。每个50-key P0保存150张攻击终点PNG和一张本任务累计ASR曲线；每个2-key smoke保存6张攻击终点PNG和一张曲线。10-key移除诊断单独保存在`实验结果/diagnostics/removal_beta_1p5/<run_id>/`，保存30张最终图、逐key指标、三水印汇总、哈希和一次总结，不保存检查点PNG。
...
- `data_role_overlap.csv`及其完整性检查结果，能够证明200张伪造目标与5000张clean-prior池的预注册交集和角色分配；
- 3 个预注册可视化 key 在全部实验条件中的最终攻击图、常规变换图和匹配噪声控制图；
- 3个预注册可视化key在迭代条件中的每150步PNG和任务级最终预算PNG；其他key不永久保存最终图或中间图；
- 全部200 keys的任务级最终预算评价指标、最终输出SHA-256和最终完成状态；
...
最终逐key指标表每行至少记录`task_budget_name`与对应`final_step`，并可追溯到协议、run、条件、样本、参数、seed、最终扰动、检测、质量和时间。中间损失和中间指标不进入该表。

## 12. 执行顺序与验收门槛

1. 冻结本版本协议、1500步双任务预算、正式beta网格和 key manifest，计算哈希，并验证历史P0样本与正式 200-key manifest 不重叠。
2. 实现三个水印在SDv1.4同模型与SDv2.0跨模型两种正式组合下的统一接口。锁定SD1.4、SD2-base、代理VAE revision和检测阈值。
3. 将攻击与离线评价物理拆分，增加禁止检测器调用的测试。
4. 完成全部模型、tokenizer、scheduler、水印代码、LPIPS、Inception/FID 资源和数据清单的离线资产准备与预检。
5. 历史beta=1.5诊断与已完成伪造P0只读保留；根据用户决定，不再运行其他P0或50-key固定预算确认。
6. 在实际正式 GPU 上完成运行时间基准，生成包含1500步攻击、离线评价和轨迹I/O的 ETA 与磁盘报告，经用户确认后启动全量任务。
7. 每个正式实验批次先执行一次 2-key smoke gate，验证资产、1500步预算、参数、最终图、断点恢复和最终评价链路；通过后同一工作流直接启动 200-key 正式运行。smoke 结果不进入正式统计。
8. 完成 E0，确认数据清单和原始检测率，不按检测结果更换参考图。
9. 依次运行 E1/E2 主实验、E3 $\lambda$、E4 $N$、E5 $\beta$、E6 常规变换、E7 匹配噪声评价；复用重叠条件。
10. 所有攻击完成后对1500步最终输出运行离线检测和质量评价；对E3–E5 Proposed相关条件额外离线评价步骤0、100、…、1500临时检查点的p值或bit accuracy，生成第10.2节五组轨迹图后清理临时图像。
11. 每个 run 只生成一次最终总结与哈希校验；失败或不完整 run 同样保留一次状态总结。

### 12.1 每批实验的自动 smoke gate

- “每次实验”定义为一个具有独立 resolved-config 哈希的批次。一个批次可以包含同一实验组内预注册的多种方法或参数条件，但不能跨越不同代码 SHA、资产锁或样本 manifest。
- 历史伪造P0与beta=1.5诊断的smoke及结果只读保留，不再作为v1.15的待执行门禁；v1.15只要求每个正式批次运行`key_000/001`正式smoke。
- 正式批次启动命令先自动生成仅含`key_000`、`key_001`的smoke子配置，其任务级预算必须与随后200-key配置一致。
- smoke必须实际跑完该批次包含的所有方法/条件，不只调用`--help`或加载模型。普通正式批次smoke生成参数、最终指标和验收报告；E3–E5 Proposed批次smoke必须额外验证“每100步临时检查点→独立离线p值/bit accuracy→轨迹CSV→线型图→临时图清理”链路。
...
- 攻击日志和进程依赖中不存在检测器调用；
- 最终输出固定为任务级预算；同一任务内所有正式迭代方法实际完成相同预算；
...
- 数据角色审计通过：200张伪造目标和5000张clean prior均来自`val2017`，交集恰为200；200个clean-prior组各25张、两两不相交、并集恰为5000；
...
latent_space_aggregation_attacks/
  ...
  scripts/
    main_methods/
      run_forgery.py
      run_removal.py
      run_forgery_budget_pilot.py
      run_removal_budget_pilot.py
      run_removal_beta_diagnostic.py
    baselines/
      run_jain_forgery.py
      run_jain_removal.py
...
`run_forgery_budget_pilot.py`与`run_removal_budget_pilot.py`分别负责两类P0，`run_removal_beta_diagnostic.py`负责10-key固定预算诊断；三者输出物理隔离。`run_formal_batch.py`必须验证两个任务预算均已冻结，再进入正式链路。
