...
- Origin Mode: plan
- Protocol Version: `formal_protocol_v1.11`
...
- P0对每个key和水印使用64个预注册候选中的最先5张有效参考；五张必须全部通过对应水印正式阈值，否则该批次失败。选择规则和全部已检测候选写入`reference_selection_control.csv`，选定图像的SHA-256进入结果校验和。
...
P0 单独保存在 `实验结果/budget_selection_pilot/<run_id>/`，至少包含协议/配置快照、50-key pilot manifest、`pilot_first_success.csv`、按任务/水印/模型设置分组的 `pilot_asr_by_step.csv`、伪造与移除 ASR 曲线、`pilot_fixed_budget_confirmation.csv`、时间记录、校验和及一份预算选择总结。P0 文件不得复制进正式 run 或与正式 200-key 指标合并。
