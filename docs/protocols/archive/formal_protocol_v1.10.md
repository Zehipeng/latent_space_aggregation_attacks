...
- Origin Mode: plan
- Protocol Version: `formal_protocol_v1.10`
...
- 数据隔离：P0的100个pilot key、伪造攻击目标身份和参考候选清单不得与正式200-key测试集的对应角色重叠。作为公开无标签辅助输入的`val2017` clean-prior池允许按第4.1节规则跨任务、跨阶段复用；P0攻击输出和攻击成功结果不得带入正式实验。参考有效性选择只能使用统一的预注册候选顺序和正式阈值。使用`master_seed=205`，子种子额外加入`budget_pilot`命名空间。
